...
const MOCK_KEYWORDS: Record<CategoryId, string[]> = {
  fire: ['fire', 'smoke', 'flame', 'burning', 'burnt', 'gas leak', 'blaze', 'explosion', 'cylinder'],
  road_accident: ['road accident', 'accident', 'collision', 'crash', 'hit and run', 'vehicle hit', 'bike fell', 'overturn', 'rammed'],
  missing_person: ['missing person', 'missing', 'lost child', 'disappeared', 'kidnap', 'untraceable', 'not returned home'],
  medical: ['medical', 'collapse', 'unconscious', 'injured', 'ambulance', 'bleeding', 'heart attack', 'fainted', 'emergency patient'],
  tree_fall: ['tree fall', 'tree fell', 'fallen tree', 'branch fell', 'uprooted', 'tree blocking', 'tree collapsed'],
  road_damage: ['road damage', 'road damaged', 'damaged road', 'broken road', 'road broken', 'pothole', 'pot hole', 'crack', 'cracked road', 'footpath', 'sunken road', 'caved', 'bad road', 'crater', 'flyover crack'],
  public_nuisance: ['public nuisance', 'nuisance', 'loudspeaker', 'loud noise', 'noise', 'encroach', 'illegal parking', 'disturbance', 'eve teasing'],
  electricity: ['electricity', 'electric', 'power cut', 'power', 'spark', 'sparking', 'pole', 'wire', 'shock', 'transformer', 'short circuit', 'no current'],
  water: ['water', 'pipeline', 'pipe burst', 'leak', 'leakage', 'sewage', 'flood', 'no water', 'contaminated water', 'drainage overflow'],
  garbage: ['garbage', 'trash', 'waste', 'rubbish', 'dump', 'drain', 'sanitation', 'sewage smell', 'not collected', 'dead animal'],
}
...
function mockSuggest(description: string): AiSuggestion {
  ...
  const confidence = category
    ? Math.min(0.9, 0.55 + bestScore * 0.12)
    : 0.3
  ...
  return {
    ...
    title,
    summary: description.trim().slice(0, 160) || (category ? CATEGORIES[category].name : ''),
