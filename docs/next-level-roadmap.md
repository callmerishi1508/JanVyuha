...
**Phase D2 — Reach channels (≈2–3 days, do after D1): DONE except Telegram (2026-07-14).**
...
- **P1-4** DPDP right to erasure shipped alongside: `api/delete-account.ts` (token-verified self-serve deletion, anonymises past reports), `api/retention-sweep.ts` (daily cron, 90-day PII age-out), `deleteAccount()` + MyIssues UI.
- **P2-9** public Transparency: department resolution leaderboard (gated by `VITE_PUBLIC_LEADERBOARD`), "most-supported open issues", CSV + JSON open-data downloads (`toPublicJson()` explicit allow-list with a PII-leak regression test); `api/weekly-digest.ts` Vercel Cron (Mon 04:23) emails a 7-day summary to `DIGEST_RECIPIENTS`.
- **P2-10** Gemini upgrades: `mode:'translate'` on `api/analyze.ts` (shared `callGemini()` helper) + stakeholder/admin "Translate" toggle on IssueDetail (lazy, cached, never on the submit path). (b) flagged→moderation auto-routing and (c) EXIF strip via canvas re-encode were already in place — verified.
- **P2-11** `lib/speech.ts` `useSpeechInput()` over the Web Speech API (en/hi/te/ta-IN follows the UI language); mic toggle on the wizard description step; hides itself where unsupported.
- **P2-12** completed: category QR posters minted client-side (`qrcode` + canvas) in a new admin Outreach tab; each deep-links `/report?category=<id>`. (Deep links + `/embed` widget were already shipped.)
- **P2-13** `api/geocode.ts` proxy (compliant User-Agent + contact, 1 req/s pacing, CDN `s-maxage`, Photon fallback) + client rounding (~11m) with memory/localStorage caching in `lib/geocode.ts`.
- **Deferred:** P2-7 Telegram bot (user decision 2026-07-14 — biggest lift, optional channel); P2-14 virtualization (roadmap: "note, not urgent").
